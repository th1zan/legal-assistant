...
## Outils d'accès programmatique

### Priorité d'accès

Pour l'accès automatisé aux bases de données, utiliser dans cet ordre de préférence :

1. **CLI** (scripts locaux) : Plus fiable, pas de dépendance réseau temps réel
2. **MCP** (servers intégrés) : Interface unifiée pour l'agent
3. **API** (REST/SPARQL) : Accès direct aux sources
4. **Web** (fetch) : Dernier recours

### Outils disponibles

| Base | Outil | Type | Commande/Usage |
|------|-------|------|----------------|
| **Fedlex** | `fedlex_sparql.py` | CLI | `python fedlex_sparql.py search "protection données"` |
| **bger.ch / jurisprudence** | `entscheidsuche` | MCP | `search_decisions(query="...", court="CH_BGer")` |
| **Commentaires** | `onlinekommentar` | MCP | `search_commentaries(query="...")` |
| **Législation cantonale** | Sites officiels | Web | Voir skill `@sources-cantonales` |

### Références

→ Documentation complète des outils : skill `@outils-recherche-juridique`
→ Workflows jurisprudence : skill `@swiss-case-law-research`
→ Workflows commentaires : skill `@swiss-legal-commentary`
